<script>
  ...
  import Input from '../molecules/NumericInput.Timer.svelte';
  ...
  $: stabilizationMode = $serialData.state.stabilizationMode;
...
<div class="row">
  <div class="column">
    <span class="emphasize">{$serialData.batVoltage}</span>
  </div>
  <div class="column">
    <span class="emphasize">{$serialData.batCurrent}</span>
  </div>
</div>
<div class="row">
...
<div class="row">
  <div class="column">
    <Input attrs={{ label: 'Вентилятор 45% + ', units: '%' }} />
  </div>
  <div class="column">
    {$serialData.fanRPM}
    об/мин
